...
def parse_args():
    ...
    parser.add_argument('--rendered_results_path', type=str, default=None, help="path to the result, like: /home/shijie/Desktop/work/feature-4dgs/output/italian_pup/native_feat_davis.yaml_compactgs_mixfeat_nomotion_channel32_dep=uni_gt_cam=False_lrfeat=0.01_reversed=False_20241111_004805/final_viz/99_round_freezing/rendered_results.pth",)
...
def segmentation(semantic_head_path=None,video_dir=None, rendered_results_path=None, rendered_feature_path=None , head_config="../configs/default_config.yaml", ann_frame_idx=0, point=None, box=None, save_dir=None):
    ...
    if point is None and box is not None: # only box input
        ...
        plt.show()
        save_dir = os.path.join(save_dir, "sam2_segmentation_box")
        ...
        show_mask((out_mask_logits[0] > 0.0).cpu().numpy(), plt.gca(), obj_id=out_obj_ids[0])
        plt.show()
        save_dir = os.path.join(save_dir, "sam2_segmentation_point")
        ...
        plt.show()
        save_dir = os.path.join(save_dir, "sam2_segmentation_point&box")
...
if __name__ == "__main__":
    args = parse_args()
    segmentation(semantic_head_path=args.semantic_head_path, video_dir=args.video_dir, rendered_results_path=args.rendered_results_path, head_config=args.head_config, ann_frame_idx=args.frame, point=args.point, box=args.box)
