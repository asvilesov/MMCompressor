...
from mem_forward_update import PhiCompressor
from dataset_utils import CustomEvaluationCallback
        ...
        with open(f"{folder_path}/sample_outputs_{global_step}.txt", "w") as f:
            for i in range(num_samples):
                ...
                for t in range(num_timesteps):
                    ...
                    f.write(f"GT: {sample_inputs[i][t_str]['A_str'][0]}\n")
                    f.write(f"Output: {sample_outputs[i][t_str]}\n")
                f.write("-------------\n")
...
# Load the dataset
dataset = load_from_disk("mem_dict_update")
